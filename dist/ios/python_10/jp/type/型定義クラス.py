...
Set = Set
範囲クラス = 範囲クラス
数理的条件クラス = 方程式クラス | 不等式クラス | 非等式クラス | 複合条件クラス | 数理的真偽クラス
項クラス = 式クラス | 有限小数クラス | 整数クラス | 分数クラス
...
テーブルdata = dict[str, list[str | int | float]]
...
single_condition = 方程式クラス | 非等式クラス | 不等式クラス | 不等式_v1クラス | 不等式_v2クラス | 数理的真偽クラス
数理的範囲 = 方程式クラス | 非等式クラス | 不等式クラス | 不等式_v1クラス | 不等式_v2クラス | 複合条件クラス | 複合条件_v1クラス | 複合条件_v2クラス
数理的コンディション = 数理的範囲 | 数理的真偽クラス
...
class add_and_subtract_products_equation(TypedDict):
    ...
    right_constant_term: 整数クラス | 分数クラス
...
説明 = 章クラス | 関係コンテンツクラス | str | マークダウンコンテンツクラス | テーブルクラス | 図形登録クラス | ベン図登録クラス
...
項_インスタンス = float | int | 整数クラス | 有限小数クラス | 分数クラス | 式クラス
...
term_ = 項クラス | float | int | NativeFraction
項 = 項クラス | str | float | int | NativeFraction
...
関係コンテンツ = 数理的コンディション | 項クラス | 数理的命題クラス | 解ずクラス | ベクトル方程式クラス | ベクトル解ずクラス | ベクトルクラス

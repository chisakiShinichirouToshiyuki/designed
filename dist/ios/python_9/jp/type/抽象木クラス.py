...
class 抽象木クラス:
    ...
    def __init__(self, 証明: 型定義クラス.証明クラス, 表現形: str, チェック対象の演算子: list[型定義クラス.symbol_type] = [], 変数タイプ:  Union[型定義クラス.変数タイプオプション , 型定義クラス.変数タイプずオプション]  = {}):
        pass
    ...
    def 取得する_終端(self, 抽象木:  Union[型定義クラス.抽象木要素 , None]  = None, leaf_type: list[型定義クラス.演算子] = [], leafs: list[型定義クラス.抽象木要素] = []) -> list[型定義クラス.抽象木要素]:  # type: ignore
        pass

    def 取得する_乗数項(self, 抽象木:  Union[型定義クラス.抽象木要素 , None]  = None) -> list[str]:  # type: ignore
        pass

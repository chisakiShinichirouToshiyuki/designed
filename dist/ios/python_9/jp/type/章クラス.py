...
class 章クラス:
    ...
    def 作成する_下位の章(self, タイトル: str, id: str, index:  Union[int , None]  = None) -> 章クラス:  # type: ignore
        pass
    ...
    def __setitem__(self, index: int, 値:  Union[章クラス , 型定義クラス.関係コンテンツクラス] ) ->  Union[章クラス , 型定義クラス.関係コンテンツクラス] :  # type: ignore
        pass
    ...
    def 追加する(self, コンテンツ:  Union[型定義クラス.説明 , 型定義クラス.関係コンテンツ] , 順接かい: bool = False, 補足: str = '', 補足タイプ: Literal['because', 'annotate', 'plain'] = 'because') -> 章クラス:  # type: ignore
        pass

    def 挿入する_章(self, 章:  Union[章クラス , str] )->章クラス:  # type: ignore
        pass
